document.addEventListener("DOMContentLoaded", function () {
    ...
    function disableReservedSlots(selectedDate) {
        ...
        const nonReservableForDateAndLocation = nonReservableSlots
        .filter(slot => slot.location === "30, rue de Trévise 75009 Paris" && slot.date === selectedDate)
        .map(slot => slot.time);
        ...
        document.querySelectorAll(".slot").forEach(slot => {
            ...
            const isBlocked = blockedSlotsForDateAndLocation.includes(slotTime);
            const isReserved = reservedSlots.includes(slotTime);
            const isNonReservable = nonReservableForDateAndLocation.includes(slotTime);
